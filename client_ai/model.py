    ...
    def preprocessing(self, x):
        ...
        for batch in range(batch_size):
            for i in range(self.frames):
                feature_vector[batch, :, self.n_mels * i: self.n_mels * (i + 1)] = x[batch, :,
                                                                                   i: i + self.vector_array_size].T
...
class Wavegram_AttentionModule(nn.Module):
    def __init__(self, h):
        ...
        # classifier
        # self.classifier = MobileFaceNet(num_class = 41) /////////////////////////////////////////////// 2025.0716.21:31
        self.classifier = MobileFaceNet(num_class=12)
        # self.arcface = ArcMarginProduct(in_features = self.h, out_features = 41, s = 40, m = 0.7)
        self.arcface = ArcMarginProduct(in_features=self.h, out_features=12, s=40, m=0.7)
...
class Wavegram_AttentionMap(LightningModule):
    ...
    def __init__(self, h, lr):
        ...
        self.accuracy_training = Accuracy(task="multiclass", num_classes=12)
        self.accuracy_val = Accuracy(task="multiclass", num_classes=12)
        self.accuracy_test = Accuracy(task="multiclass", num_classes=12)
    ...
    def mixup_data(self, x, y, alpha=0.2):
        y = torch.nn.functional.one_hot(y, num_classes=12)
        if alpha > 0:
    ...
    def configure_optimizers(self):
        ...
        return {
            "optimizer": opt,
            "lr_scheduler": {
                "scheduler": torch.optim.lr_scheduler.CosineAnnealingLR(optimizer=opt, T_max=300,
                                                                        eta_min=0.1 * float(self.lr))
            },
        }
...
if __name__ == "__main__":
    ...
    # metadata = torch.nn.functional.one_hot(torch.randint(low = 0, high = 41, size =(16,)), num_classes=8)
    metadata = torch.nn.functional.one_hot(torch.randint(low=0, high=12, size=(16,)), num_classes=12)
